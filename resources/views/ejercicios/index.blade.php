<x-app-layout>
                    ...
                    <div class="flex ">
                        <div class=" xl:w-96 ml-6">
                            ...
                            <div class="input-group relative flex flex-wrap items-stretch w-full mb-4">
                                <input name="busqueda" id="search" value="{{ $request->busqueda }}"
    ...
    <div class="oscurecer grid  md:grid-cols-3 lg:grid-cols-4 sm:grid-cols-2"
        ...
        @if ($ejercicios->count() == 0 || $ejercicios->count() == 1)
            ...
            @else
                <div class="  md:col-span-3 lg:col-span-1 lg:h-96"
                    style="display: grid; grid-gap:0; grid-template-columns:repeat(auto-fill,minmax(220px,1fr));">
        @endif
        ...
        @foreach ($tipos as $tip)
            <a href="{{ route('ejercicio.index1', $tip) }}" class="oscurecer">
                <div style="display:block;contain:style; color:#3c4043">
                    <div
                        style="display:flex;flex:1 1 220px; overflow: hidden;padding: 7px;position: relative; vertical-align: top; max-width: 100%;min-width: 0;outline: none; color: rgba(0, 0, 0, 0.72); touch-action: pan-x pan-y;">
                        <div style="display: flex;flex:1 1 auto;outline:none;overflow: hidden;">
                            <div
                                style="background-color: #fff;border:1px solid #dadce0;border-radius: 6px;margin-bottom: 10px;display: flex;flex: 1 1 auto;flex-direction: column;overflow: hidden;position: relative;">
                                <div style="display: flex; flex:0 0 48px;">
                                    <i class="fa-solid fa-folder mt-1"
                                        style="flex: auto;height:24px;padding: 12px 16px;position: relative; width: 24px;z-index: 1;"></i>
                                    <div
                                        style="align-self: center;box-sizing:border-box;flex: 1 1 auto;font-weight: 500;overflow: hidden;padding-right: 12px;position: relative; text-overflow: ellipsis; white-space: nowrap;width: calc(100%-56px);">
                                        {{ $tip->nombre }}
                                    </div>
                                    <div
                                        style="align-self: center;box-sizing:border-box;flex: 1 1 1;font-weight: 500;overflow: hidden;padding-right: 12px;position: relative; text-overflow: ellipsis; white-space: nowrap;width: calc(100%-56px);">
        ...
        <a href="{{ route('ejercicio.index2') }}" class="oscurecer">
            <div style="display:block;contain:style; color:white">
                <div
                    ...
                    <div style="display: flex;flex:1 1 auto;outline:none;overflow: hidden;">
                        <div
                            style="color:#fff;background-color: black;border:1px solid #dadce0;border-radius: 6px;margin-bottom: 10px;display: flex;flex: 1 1 auto;flex-direction: column;overflow: hidden;position: relative;">
                            <div style="display: flex; flex:0 0 48px;">
                                <i class="fa-solid fa-folder mt-1"
                                    style="flex: auto;height:24px;padding: 12px 16px;position: relative; width: 24px;z-index: 1;"></i>
                                <div
    ...
    <div class=" sm:col-span-4 lg:col-span-3 mx-w-sm oscurecer " id="pdfEjercicios">
        ...
        <form name="bus" action="{{ route('ejercicio.index2') }}" method="get">
            ...
            @else
                @if (isset($tipo))
                    <h2 style="border-radius: 10px;background-color:black;text-align: center;width: 70%;margin: auto;"
                        class="mb-3 text-white font-bold p-2 text-2xl">Ejercicios de ({{ $tipo->nombre }})</h2>
                @else
                    <div style="display: flex">
                        <h2 style="border-radius: 10px;background-color:black;text-align: center;width: 70%;margin: auto;"
                            class="mb-3 text-white font-bold p-2 text-2xl">Ejercicios Publicados</h2>
                        <div class="ml-4 mt-1">
                            <select name="tipo_id"
                                class="ml-2 py-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full  sm:text-sm border-gray-300 rounded-md">
                                <option value="-10" class="text-base font-bold"
                                    @if ($request->tipo_id == -10) class="text-base font-bold" selected @endif>
                                    Cualquiera...</option>
                                @foreach ($tipos as $item)
                                    <option value="{{ $item->id }}" class="text-base font-bold"
                                        @if ($item->id == $request->tipo_id) selected @endif>{{ $item->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="ml-4 mt-1">
                            ...
                            <button type="submit"
                                class="bg-black hover:bg-gray text-white font-bold py-2 px-4 rounded"><i
        ...
        @foreach ($ejercicios as $item)
            <div style="border:1px solid #dadce0;border-radius: 6px;margin-bottom: 10px;"
                class="  p-2 rounded bg-white shadow-lg ml-3">
                <div class="text-lg flex flex-row my-2" style="background-color: #0288d1;color:white;">
                    ...
                    @if (isset($sesion))
                        <img src="{{ asset('storage/' . $item->sesion->team->escudo) }}"
                            class="mr-3 h-12 mx-auto my-auto flex-end" style="width:60px;height:60px">
                    @else
                        <img src=" @if (isset($item->user->profile_photo_path)) {{ asset('storage/' . $item->user->profile_photo_path) }} @else {{ asset('storage/players/avatar.png') }} @endif"
                            class="mr-3 h-12 mx-auto my-auto flex-end rounded-full object-cover w-10 h-10">
                    @endif
                ...
                <div class="grid-ejercicio" style="padding: 0;">
                    ...
                    <div style="border:solid 1px #0288d1;" class=" rounded p-2 formulario">
                        <div style=" padding: 10px; display:flex; "
                            class="flex sm:flex-col md:flex-row justify-center text-center mx-auto">
                            <div class="grid-formulario" style="align-items: center;padding: 0;margin-right:5px">
                                    ...
                                    <div style="padding-top: 10px;grid-template-columns: 6em 1fr;align-items: center;">
                                        ...
                                        <div>
                                            <input disabled id="created_at" name='created_at'
                                                style="color:gray;padding: 9px 10px;width: 100%;text-align:center; display: block;margin: 0;outline: medium none; border-bottom: solid 1px #c6d6df;"
                                                value="2022-23-05" />
                                        </div>
                                    </div>
                                    <div style="padding-top: 10px;grid-template-columns: 6em 1fr;align-items: center;">
                                ...
                                <div class=" mr-1 mb-2">
                                    <button id="botonEditar" data-modal-toggle="modalEditar"
                                    ...
                                    @if (isset($sesion))
                                    <div class=" mr-1 mb-2">
                                        <form action="{{ route('ejercicios.quitar', $item) }}" method="GET"
                                            enctype="multipart/form-data">
                                            @csrf
                                            <button
                                                class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 rounded mx-auto lg:whitespace-nowrap"
                                                style="width:50%;"
                                                onclick="return confirm('Quitar el ejercicio {{ $item->nombre }} de la sesion {{ $sesion->nombre }}?')">
                                    ...
                                    @else
                                    <div class=" mr-1 mb-2">
                                        <form action="{{ route('ejercicios.destroy', $item) }}" method="GET"
                                            enctype="multipart/form-data">
                                            @csrf
                                            <button
                                                class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 rounded mx-auto lg:whitespace-nowrap"
                                                style="width:50%;"
                                                onclick="return confirm('Borrar el ejercicio {{ $item->nombre }} permanentemente ?')">
                                                <i class="mr-1 fa-solid fa-trash"></i><span
